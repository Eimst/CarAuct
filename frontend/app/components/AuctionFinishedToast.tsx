import React from 'react';
import {Auction, AuctionFinished} from "@/types";
import Link from "next/link";
import Image from "next/image";
import {numberWithCommas} from "@/app/lib/numberWithComma";


type Props = {
    finishedAuction: AuctionFinished;
    auction: Auction;
}

function AuctionFinishedToast({auction, finishedAuction}: Props) {
    return (
        <div>
            <Link href={`/auctions/details/${auction.id}`} className={`flex flex-col items-center`}>
                <div className={`flex flex-row items-center gap-2`}>
                    <Image
                        src={auction.imageUrl}
                        alt={`image of car`}
                        height={80}
                        width={80}
                        className={`rounded0lg w-auto h-auto`}
                    />
                    <div className={`flex flex-col`}>
                         <span>
                             Auction for {auction.make} {auction.model} has been finished
                             {finishedAuction.itemSold && finishedAuction.amount ? (
                                 <p>Congrats {finishedAuction.winner} who has won this auction for ${numberWithCommas(finishedAuction.amount)}</p>
                             ) :
                             <p>This item did not sell</p>
                             }
                        </span>
                    </div>

                </div>
            </Link>
        </div>
    );
}

export default AuctionFinishedToast;